...
class IndexPage(WebPage):
    def __init__(self, url, encoding = 'utf-8'):
        ...
        WebPage.__init__(self, Url.fullUrl(Url, self.allCategoreUrl), encoding = encoding)
        print('IndexPage')
        self.getIndex()
...
class Chapter:
    ...
    def analyzeTitle(title):
        print(title)
        titleRule = re.compile(r'((?P<chapter_id>\d+?)\.)?\s*(?:(?P<part_order>第[零〇一二两三四五六七八九十百千万零壹贰叁肆伍陆柒捌玖拾佰仟万\d]+[卷章节回]?)\s*(?P<part_name>\S+?\s*))??(?:(?P<chapter_order>第[零〇一二两三四五六七八九十百千万零壹贰叁肆伍陆柒捌玖拾佰仟万\d]+[卷章节回]?)\s*)?(?P<chapter_title>\S+?)\s*$')
...
class Novel:
    ...
    def getChapter(self, chapterOrder, watch = True):
        ...
        for i in range(5):
            try:
                self.chapter = Chapter(self.site, *self.index[chapterOrder], watch)
                self.chapter.order = chapterOrder % self.chapterNum + 1
                succeed = True
                break
            except:
                pass
